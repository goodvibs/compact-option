...
It targets the same core use case: compact option storage that uses exactly as much memory as raw `R`, intended for `R` values with spare bit patterns (primarily `#[repr(u8)]` enums with fewer than 256 variants).
